...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//解除JTAG在PA15的映射，恢复PA15的GPIO功能
	...
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_2;					//CH1映射到PA15了 	
	...
	TIM_TimeBaseInitStruct.TIM_Period = 100 - 1;					//设置ARR寄存器 计数器目标值   0-65535
	TIM_TimeBaseInitStruct.TIM_Prescaler = 18 - 1;					//PSC寄存器 预分频取值	0-65535
	...
	TIM_OCInitStruct.TIM_Pulse = 0;							//设置CCR寄存器
